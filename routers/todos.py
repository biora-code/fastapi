...
import models
from database_copy import engine, SessionLocal
...
templates = Jinja2Templates(directory="templates") #connect it to the templates directory
...
async def add_new_to_do(request: Request):
    user = await get_current_user(request)
    if user is None:
        return RedirectResponse(url="/auth", status_code=status.HTTP_302_FOUND)
...
async def  edit_to_do(request: Request,todo_id: int, db: Session = Depends(get_db)):
    user = await get_current_user(request)
    if user is None:
        return RedirectResponse(url="/auth", status_code=status.HTTP_302_FOUND)
...
async def  edit_to_do_commit(request: Request,todo_id: int, title:str = Form(...),description: str = Form(...),priority : int = Form(...),db: Session = Depends(get_db)):
    user = await get_current_user(request)
    if user is None:
        return RedirectResponse(url="/auth", status_code=status.HTTP_302_FOUND)
...
async def complete_todo(request: Request,todo_id: int,db: Session = Depends(get_db)):
    ...
    todo.complete = not todo.complete  #switching it to the oppositeof what it is
